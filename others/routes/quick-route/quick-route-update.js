...
import { Icon, Heading, Page, Card, Button, TextField, Modal, Subheading, ResourceList, Checkbox, ResourceItem, Stack, TextStyle, Badge, Select, Tag, Link, TextContainer } from "@shopify/polaris";

import { DragHandleMinor, DeleteMajor, PrintMinor } from "@shopify/polaris-icons";
...
const Route = () => {
    ...
    const updateRoute = () => {
        if (routeTitle) {
            if (selectedOrders?.length) {
                ...
                else { constructTostStates('Please assign a delivery agent'); }
            } else {
                activateOrdersModalHandler();
                constructTostStates("Select orders to create routes");
                showLoader(false);
            }
        } else { constructTostStates('Please provide route name'); }
    }
    ...
    const activateOrdersModalHandler = () => {
        ...
        setSearching(true);
        adminApiRequestService.getAdminOrderSearch('', '', "unallocated", routeId, (responseArray) => {
            if (responseArray) {
                // responseArray.forEach((response) => {
                //     if (!response.location && !response.shipping_address?.latitude) {
    ...
    const handelCheckBox = useCallback((orderId) => {
        ...
        if (selectedOrders_orders_ids_arr.includes(orderId)) {
            ...
            orderListSearch.forEach((zoneAllOrder) => {
    ...
    const handleSelectAll = () => {
        ...
        orderListSearch.forEach((listOrder) => {
    ...
    // * For printing route details
    const navigateToRoutePrint = () => {
        window.open(`/admin/routes/quick-route/print-quick-route?routeId=${routeId}`, '_blank');
    }
    ...
    return (
        <SideNavBar>
            <Page title="Route Planner"
                ...
                secondaryActions={[{ content: 'Print/Save', size: "slim", icon: PrintMinor, onAction: navigateToRoutePrint }]}
                fullWidth>
                <style jsx>{`
                    table {
                ...
                <Stack alignment="trailing" >
                    ...
                    <Stack.Item alignment="baseline">
                        <Button primary onClick={updateRoute}>Update Route</Button>
                    </Stack.Item>
                </Stack>
                ...
                >
                    <Card sectioned>
                        ...
                        {searchTriggred && <div style={{ paddingBlock: 10 }}><Tag onRemove={handleClearButton}>Search result : Found {orderListSearch.length} orders by {remove_(selectedSearchOption, '')} '{searchText}' </Tag></div>}
                        <ResourceList
                            loading={modalLoading}
                            emptyState={orderListSearch.length == 0 && <EmptyResourceList heading="No orders found" content="There are no orders associated to this zone yet or there are no orders available for route creation" />}
                            resourceName={{ singular: 'order', plural: 'orders' }}
                            items={orderListSearch?.map((zoneOrder, index) => {
                                return {
                                    id: zoneOrder._id,
